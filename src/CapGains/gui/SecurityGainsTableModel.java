 ...
 */
public class SecurityGainsTableModel extends AbstractTableModel implements GainColumnFinder {

	private String[] columnNames = { "Security", "Gain" };

	final static int COL_SECURITY = 0;
	...
	private Vector getDataRow(Object row) {
		...
		SecurityGain gain = (SecurityGain) row;
		Vector v = new Vector();
		v.addElement(new String(gain.ticker)); //0
	...
	public Integer getTotalGain(RowFilter<AbstractTableModel, Integer> filter) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			if ((filter != null) && (filter.include(entry) == false))
				continue;
			Integer gain = (Integer) entry.getValue(SecurityGainsTableModel.COL_GAIN);
			totalGain += gain;
		}
